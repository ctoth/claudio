...
> Early design exploration. Not authoritative — see `CLAUDE.md` and `docs/` for
> current behavior. Several flags and config paths described below
> (e.g. `--set-soundpack`, `--list-soundpacks`, `~/.claudio/config.json`) never
> shipped.
